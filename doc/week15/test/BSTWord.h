...
/*-----------Change when datatype change BEGIN--------------- */

int compare(dataTypeBST rootData, dataTypeBST data) {

	if ( strcmp(rootData.content, data.content) >= 1 ) {
		return(1);
	}
	else if (strcmp(rootData.content, data.content) == 0) {
		return(0);
	}
...
void iterateBST(BSTNode *root) {

	if (root == NULL) return;

	iterateBST(root->left);
	printf("%-20s%d\n", root->data.content, root->data.n);
	iterateBST(root->right);
}


/*-----------Change when data change END-----------------------*/
